    ...
    fs.readFile('todos.json', 'utf-8', function(err, data){
        ...
        todos.forEach(function(todo){
            if(todo.id == todo_id){
                ...
                fs.writeFile('todos.json', JSON.stringify(todos), 'utf-8', function(err){
                    return res.status(200).send("Todo successfully updated");
                });
            }
        });
        if (flag == 0){
  ...
  app.delete('/todos/:id', function(req, res){
    fs.readFile('todos.json', 'utf-8', function(err, data){
        ...
        if (initial_length != final_length){
            fs.writeFile('todos.json', JSON.stringify(todos), 'utf-8', function(err){
                return res.status(200).send("Todo item was successfully deleted");
            });
        }
        else{
            return res.status(404).send("Todo item was not found");
        }
    });
  });
